...
public class Bullets extends Projectile {
	...
	public static void makeBullet(int xposition, int yposition) {
        ...
        if (GameState.defaultchar.turnedLeft == true) {
        	...
        	angle += Math.random() * .1;
        	angle -= Math.random() * .1;
        	Bullets tempbullet = new Bullets (main, xposition+100 - Main.XOffSet, yposition+30 - Main.YOffSet, xposition+100, yposition+30, 10, 10, AssetLoader.bullet, 25, 100, angle, true);
